...
class DriveManagerAI:
    ...
    def upload_file(self, file_path, semester, degree, subject, description='', use_cloud=True):
        ...
        try:
            ...
            file_id = str(len(self._load_database()) + 1)
            ...
            if use_cloud:
                ...
                upload_result = cloudinary.uploader.upload(
                    file_path,
                    resource_type="auto",
                    folder=f"student_ai/{semester}/{degree}/{subject}",
                ...
                local_path = os.path.join(
                    self.local_storage, 
                    f"{semester}_{degree}_{subject}_{filename}"
                )
    ...
    def list_files(self, semester=None, degree=None, subject=None):
        ...
        try:
            ...
            for file_id, file_data in db.items():
                matches = True
                if semester and str(file_data.get('semester')) != str(semester):
                    matches = False
        ...
        try:
            ...
            if file_data.get('is_cloud') and not file_data.get('is_external'):
                try:
                    ...
                    public_id = '/'.join(url_parts[-4:]).split('.')[0]
    ...
    def open_file(self, file_id):
        ...
        try:
            ...
            file_data = db[file_id]
            webbrowser.open(file_data['url'])
            
            logging.info(f"File opened: {file_id}")
            return {'success': True, 'message': 'File opened in browser'}
        except Exception as e:
        ...
        try:
            ...
            for file_id, file_data in db.items():
                if (query in file_data.get('filename', '').lower() or
                    ...
                    query in file_data.get('subject', '').lower()):
                    results.append({
                        'id': file_id,
                        **file_data
                    })
    ...
    def analyze_file_with_ai(self, file_id):
        ...
        try:
            ...
            try:
                ...
                return {'success': False, 'message': 'Cannot read file (not a text file)'}
